...
// Закрыть модальное окно добавления рецепта
...
function updateSubcategories() {
    ...
    const subcategories = {
        ...
        "Ужин десерты": ["Сладкие блюда", "Напитки"]
    };
...
function addIngredientField() {
    ...
    div.innerHTML = `
        <input type="text" placeholder="Количество" style="flex: 1; padding: 8px; border: 1px solid #ddd; border-radius: 4px;">
        <input type="text" placeholder="Название ингредиента" style="flex: 2; padding: 8px; border: 1px solid #ddd; border-radius: 4px;">
        <button type="button" style="background: #dc3545; color: white; border: none; width: 30px; border-radius: 4px; cursor: pointer; font-weight: bold;" onclick="this.parentElement.remove()">-</button>
    `;
...
function saveCustomRecipe() {
    ...
    // Валидация
    if (!name) {
        alert('⚠️ Заполните название блюда!');
        return;
    }
    
    if (!category) {
        alert('⚠️ Выберите тип блюда!');
        return;
    }
    
    if (!subcategory) {
        alert('⚠️ Выберите подкатегорию!');
        return;
    }
    ...
    if (!hasValidStep) {
        alert('⚠️ Добавьте хотя бы один шаг приготовления!');
        return;
    }
    ...
    if (!userRecipes[category]) userRecipes[category] = {};
    if (!userRecipes[category][subcategory]) userRecipes[category][subcategory] = [];
    
    userRecipes[category][subcategory].push(recipe);
    ...
    // Показать уведомление и перезагрузить страницу
    alert('✅ Рецепт успешно добавлен!\nОн будет доступен в выпадающих списках.');
    location.reload();
}
